...
#define BME680_PHY_ADDRESS CONFIG_BME680_ADDR
...
altitude = 44330 * (1.0 - pow(values.pressure/100 / 1013.25, 0.1903));
pressure_raw = values.pressure;
temp_bme = values.temperature;
pressure_sea = (pressure_raw/100) / pow(1 - ((0.0065 * altitude) / (temp_bme + (0.0065 * altitude) + 273.15)), 5.257);
humidity_bme = values.humidity;
//Mozno pouzit este: values.gas_resistance
                       }
        }
...
// Init all I2C bus interfaces at which BME680 sensors are connected
    ...
    sensor = bme680_init_sensor (I2C_BUS, BME680_PHY_ADDRESS, 0);
        if (sensor)
    {
   ...
   xTaskCreate(&ota_example_task, "ota_example_task", 8192, NULL, 5, NULL);     //OTA
   xTaskCreate(&https_get_task, "https_get_task", 8192, NULL, 4, NULL);        //ZAPIS NA WEBSERVER
